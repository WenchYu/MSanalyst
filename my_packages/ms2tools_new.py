...
def ms1_match(args,queryMGF = None):
    ...
    if queryMGF is None:
        ...
        query_mz = str(quant_df.iloc[0, 1])
    ...
    edb_results = Parallel(n_jobs=n_jobs)(
        ...
        quant_df_row in tqdm(quant_df.to_dict('records')))
...
def ems2_match(args,queryMGF=None):
    ...
    col_name2 = f'{args.library_matching_method}_mps'
    edb_ms1_df[col_name], edb_ms1_df[col_name2] = np.nan, np.nan
    if not edb_ms1_df[col_name].isna().all(): # Skip if the library match method has been applied
        pass
    else:
        for i in trange(len(edb_ms1_df)):
            ...
            try: # Some query spectra have no ms1 hits
                ...
                # Output
                edb_ms1_df.loc[i, col_name] = SPEC_SIM
                edb_ms1_df.loc[i, col_name2] = N_PEAK
...
def isms2_match(args,queryMGF=None):
    ...
    col_name = f'{args.library_matching_method}_similarity0'
    col_name2 = f'{args.library_matching_method}_mps'
    is_ms1_match_df[col_name], is_ms1_match_df[col_name2] = np.nan, np.nan
    if not is_ms1_match_df[col_name].isna().all(): # Skip if the library match method has been applied
        pass
    else:
...
def mn_curating(G: nx.Graph, args) -> nx.Graph:
    ...
    if args.top_k == -1:
        ...
        for node in list(G.nodes()):
            ...
            scored_nb = [(G[node][nb].get(f'{args.self_clustering_method}_pair_similarity', 0), nb) for nb in neighbors]
            topk_nb   = {nb for _, nb in heapq.nlargest(args.top_k, scored_nb)}
