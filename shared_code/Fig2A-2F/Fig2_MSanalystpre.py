...
'''
Filtering the MSanalyst results based on the thresholds (matched peaks: 3-6; MS2 similarity: 0.1-0.9)
input: MS1 and MS2 comparison results generated by MSanalyst
The input files can be downloaded as Demo data 1 at (https://msanalyst.net/i/manuals)
The comparison result can be repeated at (https://msanalyst.net/m/Molecular) using the input files.
'''
import os
import time
...
if __name__ == '__main__':
    ...
    '''isdb_file'''
    np_file = './data/IS_MS1match_std_quant.csv'
    ...
    for j in trange(len(np_df)):
        ...
        # Find the (mps, pairsimilarity) pair with the maximum pair_similarity value
